...
pub unsafe fn init(phy_mem_offset: VirtAddr) -> OffsetPageTable<'static> {
    unsafe {
        let level_4_table = l4_tab(phy_mem_offset);
        OffsetPageTable::new(level_4_table, phy_mem_offset)
    }
}

pub unsafe fn l4_tab(phy_mem_offset: VirtAddr) -> &'static mut PageTable {
    use x86_64::registers::control::Cr3;

    let (l4_tab_frame, _) = Cr3::read();

    let phys = l4_tab_frame.start_address();
